...
void CClient2Dlg::placeOrder( bool whatIf )
{
	...
	/// @todo remove this account number
	order.account = "DU99099";
...
void CClient2Dlg::DisplayMultiline( CHScrollListBox& box, const CString& title, const CString& data )
{
	...
	if ( !data.IsEmpty() )
	{
		...
		for ( ;; )
		{
			...
			CString line = data.Mid( static_cast<int>( p - begin ), len );
...
void CClient2Dlg::CStringReplace( CString &pXml, const CString &cStr, const CString &rStr, CString &pDest )
{
	...
	/// @bug this won't work for multi-byte characters because calloc allocates bytes
	...
	do
	{
		...
		{
			lenFound = static_cast<int>( pFind - pBuf );
...
void CClient2Dlg::tickOptionComputation( TickerId tickerId, TickType tickType, double impliedVol, double delta, double modelPrice, double pvDividend )
{
	CString str, impliedVolStr( "N/A" ), deltaStr( "N/A" ), modelPriceStr( "N/A" ), pvDividendStr( "N/A" );
	...
	if ( tickType != MODEL_OPTION )
	{
		...
		tickerId, ( const char* )getField( tickType ), impliedVolStr, deltaStr );
	}
	else
	{
		...
		tickerId, ( const char* )getField( tickType ), impliedVolStr, deltaStr, modelPriceStr, pvDividendStr );
	}
	int i = m_ticks.AddString( str );
